...
module.exports = /** @class */ (function () {
     ...
     */
    function KakaoDB(packageName='com.kakao.talk', newDecrypt=true) {
    ...
    KakaoDB.prototype.get = function (table, index, args = {}) {
        try {
            ...
            if(Object.keys(this._col).indexOf(table) == -1) return null;
            let cursor = !args.query ?
                this[this._col[table].loc].rawQuery("SELECT * FROM " + table + " WHERE " + (args.id ? args.id : this._col[table].index) + " = ?", [index]) :
                this[this._col[table].loc].rawQuery(args.query, null)
            if(!args.range) {
                cursor.moveToNext();
                let ret = new DBitem(table, this._col[table]).get(cursor);
                cursor.close();
                return ret;
            }
            let ret = [];
            ...
            try {
                for(let i=0;i<args.range[0];++i) {
                    ret.push(new DBitem(table, this._col[table]).get(cursor));
                    args.range[1] ? cursor.moveToPrevious() : cursor.moveToLast()
                }
            } catch (e) {}
            cursor.close();
            return ret;
